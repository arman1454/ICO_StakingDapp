...
const Staking = ({poolDetails,sweep,setLoader}) => {
  ...
  const CALLING_FUNCTION_SWEEP = async (pool) => {
    setLoader(true);
    console.log(pool);
    const receipt = await sweep(pool)
            ...
            <div className="tab-content">
              <div className="tab-pane fade show active" id="tab-f4" role="tabpanel">
                <div className="row">
                  ...
                  <InputField size={"6"} type={"text"} title={"Token Address"} name={"amount2"} placeholder={"address"}
                    handleChange={(e)=> setToken({...token,token:e.target.value})}
                  />
                  <InputField size={"6"} type={"text"} title={"Enter Amount"} name = {"amount3"} placeholder={`${poolDetails?.contractTokenBalance} ${poolDetails?.depositToken.symbol}`} 
                    handleChange={(e)=> setToken({...token,amount:e.target.value})}
                  />

                  <ClickButton name={"Withdraw"} handleClick={()=>CALLING_FUNCTION_SWEEP(token)}/>
                </div>
              </div>
            </div>
          </div>
